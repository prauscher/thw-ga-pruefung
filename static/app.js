...
var icon_note = document.createElementNS("http://www.w3.org/2000/svg", "svg");
icon_note.setAttribute("width", 16);
icon_note.setAttribute("height", 16);
icon_note.setAttribute("fill", "currentColor");
icon_note.setAttribute("viewBox", "0 0 16 16");
for (const _d of ["M14.5 3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2z", "M3 5.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5M3 8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 8m0 2.5a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5"]) {
	var _path = document.createElementNS("http://www.w3.org/2000/svg", "path");
	_path.setAttribute("d", _d);
	icon_note.append(_path);
}
// Need jQuery-Object for cloning
icon_note = $(icon_note);
...
function _openExamineeEditModal(e_id) {
	...
	function _submit(e) {
		...
		for (var name of modal.elem.find("#names").val().split("\n").values()) {
			name = name.trim();
			if (name != "") {
				socket.send({"_m": "examinee", "i": e_id === null ? _gen_id() : e_id, "name": name, "note": modal.elem.find("#note").val(), "priority": modal.elem.find("#priority").val(), "locked": modal.elem.find("#locked").val(), "flags": flags});
			}
		}
	...
	modal.elem.find(".modal-body").append([
		...
		$("<form>").append([
			...
			$("<div>").addClass("mb-3").append([
				$("<label>").attr("for", "note").addClass("col-form-label").text("Notiz für Prüfende"),
				$("<textarea>").addClass("form-control").attr("rows", 2).attr("id", "note").val(e_id !== null && "note" in data.examinees[e_id] ? data.examinees[e_id].note : "")
			]),
			$("<div>").addClass("mb-3").append([
...
function render_examiner() {
	...
	$("#examiner-examinees").empty().append(currentAssignments.map(function (assignment) {
		...
		node.append($("<span>").addClass("d-block").text(examinee.name));
		if ("note" in examinee && examinee.note != "") {
			node.append($("<span>").append($("<small>").text(examinee.note)));
		}
		return node;
